...
//        reset();
        AgentServicesTools.register(this, "nego", "buyer");
        ...
        addBehaviour(new ReceiverBehaviour(this, -1, modele, true,(a,msg)->{
            ...
            var prices = msg.getContent().split(";");
            var counterProposal = Double.parseDouble(prices[0]);
            window.println("-> I've received a proposal of %.2f€".formatted(counterProposal));
            if (prices.length == 2) {
                var initialProposal = Double.parseDouble(prices[1]);
                window.println("---> It's an answer to this proposal %.2f€".formatted(initialProposal));
            }
        }));
